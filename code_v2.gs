// =============================================================================
// SI-PANDA BACKEND v2 (dengan dukungan Zona Game)
// -----------------------------------------------------------------------------
// File ini adalah pengganti penuh code.gs. Perubahan dibanding versi asli:
//   [GAME-1] Helper readGames_() dan readGameResults_()
//   [GAME-2] getInitData mengembalikan field `games` dan `gameResults`
//   [GAME-3] doPost menangani action "saveGameResult"
//   [GAME-4] Fungsi setupGameSheets() untuk membuat sheet Games & GameResults
//            beserta contoh data (jalankan sekali dari editor Apps Script)
//
// Cara pakai:
//   1. Di editor Apps Script, ganti seluruh isi Code.gs dengan isi file ini.
//   2. Pilih fungsi setupGameSheets di dropdown, klik Run (sekali saja).
//   3. Deploy > Manage deployments > edit > New version > Deploy.
// Jika terjadi error, kembalikan isi Code.gs dari file code.gs di repo.
// =============================================================================
...
function doGet(e) {
  ...
  if (action === "getInitData") {
    ...
    var responseData = {
      ...
      results: resultsList,
      materi: materiList,
      // [GAME-2] Data game & riwayat skor game
      games: readGames_(ss),
...
function doPost(e) {
  ...
  try {
    ...
    // --- 8. SIMPAN / UPDATE GAME (panel Kelola Game) ---
...
/**
 * Ambil sheet, buat baru dengan header jika belum ada.
 * textColumns: daftar huruf kolom yang diformat sebagai teks, misal ["B"]
 * (dipakai agar NISN berawalan 0 dan JSON tidak diubah oleh Sheets).
 */
...
/**
 * Membaca sheet Games -> array game aktif.
 * Kolom: id | type | mapel | title | duration | isActive | linkedExamId | pairs (JSON string)
 * Kompatibel skema lama 7 kolom (pairs di kolom 7, tanpa linkedExamId).
 */
function readGames_(ss) {
  ...
  rows.forEach(function(r) {
    ...
    try {
      ...
      return; // lewati game dengan data rusak agar tidak merusak tampilan siswa
    }
...
/** Membaca sheet GameResults -> array riwayat skor siswa. */
...
/**
 * [GAME-4] Jalankan SEKALI dari editor Apps Script.
 * Membuat sheet Games (dengan contoh data) dan GameResults (kosong).
 * Aman dijalankan ulang: tidak menimpa sheet yang sudah ada.
 */
function setupGameSheets() {
  ...
  if (!resultsSheet) {
    resultsSheet = getOrCreateSheet_(ss, GAME_RESULTS_SHEET, GAME_RESULTS_HEADER, ["B"]); // NISN sebagai teks
    Logger.log("Sheet " + GAME_RESULTS_SHEET + " dibuat.");
  } else {
    Logger.log("Sheet " + GAME_RESULTS_SHEET + " sudah ada, dilewati.");
  }
  ...
  if (gamesSheet) {
    ...
    if (gamesSheet.getLastRow() < 2) {
      ...
      Logger.log("Sheet " + GAMES_SHEET + " sudah ada, dilewati.");
    }
    return;
  }

  gamesSheet = getOrCreateSheet_(ss, GAMES_SHEET, GAMES_HEADER, ["A", "H"]); // id & pairs (JSON) sebagai teks
  seedSampleGames();
  try { gamesSheet.autoResizeColumns(1, 6); } catch (e) {}
}
